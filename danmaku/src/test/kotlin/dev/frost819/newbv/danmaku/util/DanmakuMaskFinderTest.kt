...
class DanmakuMaskFinderTest {
    ...
    private fun loadWebMask() =
        DanmakuMask.fromBinary(
            File(webMaskFile.toURI()).readBytes(),
            DanmakuMaskType.WebMask,
        )

    private fun loadMobMask() =
        DanmakuMask.fromBinary(
    ...
    @Test
    fun `findFrame returns frame for time at start`() =
        runTest {
    ...
    @Test
    fun `findFrame returns null for time after last segment`() =
        runTest {
            val finder = DanmakuMaskFinder()
            val mask = loadWebMask()
    ...
    @Test
    fun `findFrame returns correct frame for web mask`() =
        runTest {
            val finder = DanmakuMaskFinder()
            val mask = loadWebMask()
    ...
    @Test
    fun `findFrame returns correct frame for mob mask`() =
        runTest {
            val finder = DanmakuMaskFinder()
    ...
    @Test
    fun `reset clears cached segment`() =
        runTest {
            val finder = DanmakuMaskFinder()
            val mask = loadWebMask()
    ...
    @Test
    fun `findFrame caches segment and reuses on next call`() =
        runTest {
            val finder = DanmakuMaskFinder()
            val mask = loadWebMask()
    ...
    @Test
    fun `findFrame refetches when time moves outside cached segment range`() =
        runTest {
            val finder = DanmakuMaskFinder()
            val mask = loadWebMask()
    ...
    @Test
    fun `findFrame returns null when time in segment but no frame matches`() =
        runTest {
            val finder = DanmakuMaskFinder()
            val mask = loadWebMask()
