...
class SeasonDetailViewModelTest {
    ...
    private fun fakeSeasonDetail(
        ...
        subType = 1,
        seasonId = seasonId,
        userStatus = SeasonDetail.UserStatus(
            follow = follow,
            pay = false,
            progress = progress,
        ),
    ...
    @Test
    fun `onPlay emits NavigateToPlayer with last watched episode`() = runTest(testDispatcher) {
    ...
    @Test
    fun `onSwitchSeason loads new season detail`() = runTest(testDispatcher) {
        ...
        assertThat(viewModel.uiState.value.seasonDetail?.seasonId).isEqualTo(100)
        assertThat(viewModel.uiState.value.isFollowing).isTrue()
