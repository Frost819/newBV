...
sealed interface VideoDetailUiEffect {
    ...
    /** 跳转到 PGC 番剧详情页。 */
    data class NavigateToSeason(val epid: Int) : VideoDetailUiEffect
}
...
class VideoDetailViewModel @Inject constructor(
     ...
     */
    fun sendCoin() {
        val currentDetail = _uiState.value.detail ?: run {
    ...
    fun toggleFavorite() {
        ...
        if (isFav) {
            ...
            val defaultFolderId = _uiState.value.favoriteFolders
                .firstOrNull { it.title == "默认收藏夹" }?.id
                ...
                if (data != null) {
                    ...
                    _uiState.update {
                        it.copy(
                            ...
                            isCoined = data.coin,
                            isFavorite = data.fav,
                            videoFavoriteFolderIds = defaultFolderId?.let { id ->
