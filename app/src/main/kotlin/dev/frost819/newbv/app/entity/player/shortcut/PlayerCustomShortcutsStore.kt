...
object PlayerCustomShortcutsStore {
    ...
    fun get(): List<PlayerCustomShortcut> =
        PlayerCustomShortcutsCodec.parse(Prefs.playerCustomShortcuts)

    /** 获取快捷键映射（keyCode → shortcut）。 */
    fun getByKey(): Map<Int, PlayerCustomShortcut> =
        get().associateBy { it.keyCode }
    ...
    /** 新增或更新单个快捷键绑定。 */
    fun upsert(keyCode: Int, action: PlayerCustomShortcutAction): List<PlayerCustomShortcut> {
        val next = get()
    ...
    fun remove(keyCode: Int): List<PlayerCustomShortcut> =
        save(get().filterNot { it.keyCode == keyCode })
