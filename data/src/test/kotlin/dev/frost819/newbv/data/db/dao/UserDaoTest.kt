...
class UserDaoTest {
    ...
    @Test
    fun `getAll returns all users with correct field values`() = runTest {
        dao.insert(
    ...
    @Test
    fun `findUserByUid among multiple users returns correct one`() = runTest {
        dao.insert(
    ...
    @Test
    fun `findUserByUid with large uid value`() = runTest {
    ...
    @Test
    fun `findUserByUid with zero uid returns correct user`() = runTest {
    ...
    @Test
    fun `findUserByUid with negative uid returns correct user`() = runTest {
    ...
    @Test
    fun `insert multiple users via vararg in single call`() = runTest {
        dao.insert(
    ...
    @Test
    fun `insert duplicate uid creates separate records`() = runTest {
        dao.insert(
    ...
    @Test
    fun `user with empty string fields persists correctly`() = runTest {
    ...
    @Test
    fun `user with special characters in auth field persists correctly`() = runTest {
    ...
    @Test
    fun `delete multiple users via vararg removes all specified`() = runTest {
        dao.insert(
    ...
    @Test
    fun `delete non-existent user does not throw`() = runTest {
    ...
    @Test
    fun `getAll after partial delete returns remaining users`() = runTest {
        dao.insert(
    ...
    @Test
    fun `delete all users one by one then getAll returns empty`() = runTest {
        dao.insert(
            UserEntity(uid = 1L, username = "a", avatar = "", auth = ""),
            UserEntity(uid = 2L, username = "b", avatar = "", auth = ""),
        )
    ...
    @Test
    fun `update non-existent user does not throw and does not insert`() = runTest {
    ...
    @Test
    fun `update preserves id while changing all mutable fields`() = runTest {
    ...
    @Test
    fun `update changing lock from empty to non-empty persists correctly`() = runTest {
    ...
    @Test
    fun `update changing lock from non-empty to empty persists correctly`() = runTest {
